...
class SaleOrder(models.Model):
    ...
    def _check_move_state(self, line):
        ...
        if line.lot_id:
            ...
            if move.state != "assigned":
                raise UserError(_("Can't reserve products for lot %s") % line.lot_id.name)
        return True
    ...
    def _check_related_moves(self):
        ...
        for line in self.order_line:
            if line.lot_id:
                unreserved_moves = line.move_ids.filtered(lambda move: move.product_uom_qty != move.reserved_availability)
                if unreserved_moves:
                    raise UserError(_("Can't reserve products for lot %s") % line.lot_id.name)
